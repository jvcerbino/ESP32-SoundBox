...
if (lv_obj_get_style_pad_top(music_time,LV_PART_MAIN) > 0) lv_obj_set_style_pad_right( music_time, lv_obj_get_style_pad_right(music_time,LV_PART_MAIN) + 1, LV_PART_MAIN );
...
lv_obj_set_width( pp_img, LV_SIZE_CONTENT);  /// 24
lv_obj_set_height( pp_img, LV_SIZE_CONTENT);   /// 24
...
lv_obj_set_width( next_img, LV_SIZE_CONTENT);  /// 30
lv_obj_set_height( next_img, LV_SIZE_CONTENT);   /// 30
...
lv_obj_set_width( prev_img, LV_SIZE_CONTENT);  /// 30
lv_obj_set_height( prev_img, LV_SIZE_CONTENT);   /// 30
...
//Compensating for LVGL9.1 draw crash with bar/slider max value when top-padding is nonzero and right-padding is 0
if (lv_obj_get_style_pad_top(vol_bar,LV_PART_MAIN) > 0) lv_obj_set_style_pad_right( vol_bar, lv_obj_get_style_pad_right(vol_bar,LV_PART_MAIN) + 1, LV_PART_MAIN );

lv_obj_t *timeleft_label = lv_label_create(menu);
lv_obj_set_width( timeleft_label, LV_SIZE_CONTENT);  /// 1
lv_obj_set_height( timeleft_label, LV_SIZE_CONTENT);   /// 1
...
lv_obj_set_width( vol_img, LV_SIZE_CONTENT);  /// 24
lv_obj_set_height( vol_img, LV_SIZE_CONTENT);   /// 24
lv_obj_set_x( vol_img, -100 );
lv_obj_set_y( vol_img, -100 );
lv_obj_set_align( vol_img, LV_ALIGN_CENTER );
lv_obj_add_flag( vol_img, LV_OBJ_FLAG_CLICKABLE );   /// Flags
lv_obj_remove_flag( vol_img, LV_OBJ_FLAG_SCROLLABLE );    /// Flags
}

static const i2s_config_t i2s_config = {
        .mode = (i2s_mode_t) (I2S_MODE_MASTER | I2S_MODE_TX | I2S_MODE_DAC_BUILT_IN),  // <-- Isso aqui!
        .sample_rate = 44100,
